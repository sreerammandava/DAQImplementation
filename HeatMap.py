...
def HEATMAP(Sampling_Rate, Time_Duration):
    
    X_Iters = int(input("Points of X axis: "))
    Y_Iters = int(input("Points of Y axis: "))
    Delta = float(input("Delta (Step Size in cm): "))

    heatmap_data = np.zeros((Y_Iters, X_Iters))

    for y in range(Y_Iters):
        for x in range(X_Iters):
            
            Pos_X = x * Delta
            Pos_Y = y * Delta
            
            user_input = input(f"Move to X: {Pos_X:.1f} cm, Y: {Pos_Y:.1f} cm | Press Enter to Capture (or 'q' to quit): ")
    ...
    extent = [0, X_Iters * Delta, 0, Y_Iters * Delta]
